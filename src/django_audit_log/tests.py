...
def test_useragentutil_normalize_user_agent():
    ...
    assert info['device_type'] == 'Mobile' or info['device_type'] == 'Desktop' or info['device_type'] == 'Unknown'
    # Eskola APK
...
def test_loguseradmin_access_count():
    user = LogUserFactory()
    log = AccessLogFactory(user=user)
    admin_obj = audit_admin.LogUserAdmin(LogUser, admin.site)
    obj = user
    setattr(obj, "access_count", 1)
...
def test_loguseradmin_last_active():
    ...
    now = timezone.now()
    setattr(obj, "last_activity", now)
    assert str(admin_obj.last_active(obj))[:4].isdigit() or admin_obj.last_active(obj) == "Never"
    # No last_activity
    obj2 = LogUserFactory()
...
def test_loguseradmin_ip_addresses_used():
    user = LogUserFactory()
    log = AccessLogFactory(user=user, ip=LogIpAddressFactory())
    admin_obj = audit_admin.LogUserAdmin(LogUser, admin.site)
...
def test_loguseradmin_url_access_stats():
    user = LogUserFactory()
    log = AccessLogFactory(user=user, path=LogPathFactory())
    admin_obj = audit_admin.LogUserAdmin(LogUser, admin.site)
...
def test_loguseradmin_distinct_user_agents():
    user = LogUserFactory()
    log = AccessLogFactory(user=user, user_agent_normalized=LogUserAgentFactory())
    admin_obj = audit_admin.LogUserAdmin(LogUser, admin.site)
...
def test_loguseragentadmin_unique_users_count():
    ...
    admin_obj = audit_admin.LogUserAgentAdmin(LogUserAgent, admin.site)
    obj = ua
    setattr(obj, "unique_users", 2)
    assert admin_obj.unique_users_count(obj) == 2

@pytest.mark.django_db
def test_loguseragentadmin_usage_details():
    ua = LogUserAgentFactory()
    log = AccessLogFactory(user_agent_normalized=ua)
    admin_obj = audit_admin.LogUserAgentAdmin(LogUserAgent, admin.site)
    setattr(ua, "usage_count", 1)
...
def test_loguseragentadmin_related_users():
    ua = LogUserAgentFactory()
    user = LogUserFactory()
    log = AccessLogFactory(user_agent_normalized=ua, user=user)
    admin_obj = audit_admin.LogUserAgentAdmin(LogUserAgent, admin.site)
