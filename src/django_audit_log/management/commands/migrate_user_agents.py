...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Normalized logs: {initial_stats['normalized_logs']} ({initial_stats['normalized_percentage']:.1f}%)")
        self.stdout.write(f"Non-normalized logs: {initial_stats['non_normalized_logs']}")
        ...
        # Perform migration
        self.stdout.write(self.style.SUCCESS(f"Starting migration with batch size {batch_size}"))
        ...
        self.stdout.write(f"Normalized logs: {final_stats['normalized_logs']} ({final_stats['normalized_percentage']:.1f}%)")
        ...
        for browser in final_stats['top_browsers']:
            self.stdout.write(f"  {browser['browser'] or 'Unknown'}: {browser['count']}")
        ...
        for device in final_stats['device_types']:
            self.stdout.write(f"  {device['device_type'] or 'Unknown'}: {device['count']}")
            
        # Show bot percentage
        self.stdout.write(self.style.SUCCESS(f"Bot traffic: {final_stats['bot_percentage']:.1f}%"))