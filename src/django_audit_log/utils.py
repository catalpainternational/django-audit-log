...
def migrate_user_agents(batch_size=1000):
    ...
    distinct_user_agents = AccessLog.objects.filter(
    ...
    for i in range(0, total_agents, batch_size):
        ...
        print(f"Processed {processed_agents}/{total_agents} user agents, updated {updated_logs} logs")
...
def get_user_agent_statistics():
    ...
    normalized_logs = AccessLog.objects.filter(user_agent_normalized__isnull=False).count()
    non_normalized_logs = AccessLog.objects.filter(user_agent_normalized__isnull=True).count()
    
    # Count unique user agents
    unique_user_agents = LogUserAgent.objects.count()
    
    # Get top browsers
    top_browsers = LogUserAgent.objects.values('browser').annotate(
        count=Count('access_logs')
    ).order_by('-count')[:10]
    
    # Get top operating systems
    top_os = LogUserAgent.objects.values('operating_system').annotate(
        count=Count('access_logs')
    ).order_by('-count')[:10]
    
    # Get device type distribution
    device_types = LogUserAgent.objects.values('device_type').annotate(
        count=Count('access_logs')
    ).order_by('-count')
    
    # Get bot percentage
    bot_count = LogUserAgent.objects.filter(is_bot=True).aggregate(
        count=Count('access_logs')
    )['count'] or 0
    ...
    return {
        ...
        'normalized_percentage': (normalized_logs / total_logs * 100) if total_logs else 0,
